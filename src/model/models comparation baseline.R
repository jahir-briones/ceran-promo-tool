...
BaselineModel <- R6::R6Class(
  ...
  public = list(
    ...
    calculate_baseline = function(group,target_col = "Units") {
      ...
      saveRDS(self$lr_model, file = file.path("models", paste0("LR_", model_id, ".rds")))
    ...
    calculate_baseline_tree = function(group, target_col = "Units") {
      ...
      model_id <- paste(unique(group$Country), unique(group$Client), unique(group$EAN), sep = "_")
      saveRDS(self$tree_model, file = file.path("models", paste0("DT_", model_id, ".rds")))
    ...
    calculate_baseline_xgb = function(group, target_col = "Units") {
      ...
      model_id <- paste(unique(group$Country), unique(group$Client), unique(group$EAN), sep = "_")
      saveRDS(self$xgb_model,  file = file.path("models", paste0("XGB_", model_id, ".rds")))
    ...
    calculate_baseline_rf = function(group, target_col = "Units") {
      ...
      model_id <- paste(unique(group$Country), unique(group$Client), unique(group$EAN), sep = "_")
      saveRDS(self$rf_model,   file = file.path("models", paste0("RF_", model_id, ".rds")))
...
for (row in seq_len(nrow(unique_combinations))) {
  ...
  if (nrow(combo_data) > 180) {
    ...
    model_path <- file.path("models", paste0(best_baseline_result$best_model, "_", model_id, ".rds"))
    ...
    if (!table_created) {
      dbWriteTable(con, Id(schema = "ceran", table = "baseline_model_results"), compare_models, overwrite = TRUE)
      table_created <- TRUE
    } else {
      dbWriteTable(con, Id(schema = "ceran", table = "baseline_model_results"), compare_models, append = TRUE)
    }
  }
  gc()
  i <- i + 1
...
ean_baseline <- dbGetQuery(con, "SELECT * FROM ceran.baseline_model_results
                           where \"Client\" = 'Mi Farma'")
...
group <- market_df %>%
  anti_join(omit_combos, by = c("Country", "Client", "EAN"))
...
dbWriteTable(con, Id(schema = "ceran", table = "baseline_no_model_results"), compare_models_no, append = TRUE)
...
base <- dbGetQuery(con, "SELECT * FROM ceran.consolidated_baseline
...
#write.csv(base, "bases/final_baseline_costa_rica_2.csv", row.names = F)
...
# Usage  
baseline_model <- BaselineModel$new(market_df)
baseline_model$data_profiling()
baseline_model$remove_outliers("Units")
...
feature_importance <- baseline_model$get_all_feature_importance()
print(feature_importance)


vars.to.replace <- c("Baseline_LR", "r2_LR", 
                     "Baseline_DT", "r2_DT", 
                     "Baseline_XGB", "r2_XGB", 
                     "Baseline_RF", "r2_RF", 
                     "Date", "Units")
compare_models2 <- compare_models[vars.to.replace]
compare_models2[is.na(compare_models2)] <- F
compare_models[vars.to.replace] <- compare_models2

# Apply get_best_baseline function
best_baseline_results <- pmap(compare_models, function(...) {
  row <- list(...)
  baseline_model$get_best_baseline(row)
})

# Extract the best baseline and model for each row
compare_models$best_baseline <- sapply(best_baseline_results, function(x) as.numeric(x[[1]]))
compare_models$best_model <- sapply(best_baseline_results, function(x) as.character(x[[2]]))

compare_models <- ean_baseline %>% 
  group_by(Country, Client, EAN, Month) %>% 
  mutate(best_baseline = if_else(is.na(best_baseline), 
                                 median(ifelse(Discount == 0, Units, 1)
                                 ),
                                 best_baseline)
  )

table(compare_models$best_model)


# Calculate model counts and percentages
model_counts <- compare_models %>% count(best_model) %>% rename(Frequency = n)
...
#'7509552455557','3600524057336','6923700977561','3600542081160','7509552845884','7509552849516'
example_group <- merged_data %>% 
  filter(EAN == '3600542081160')

merged_data_for_viz <- left_join(example_group, compare_models, by = c("Country","Client", "EAN", "Date"))
merged_data_for_viz <- merged_data_for_viz %>%
  mutate(Date = #Date
           floor_date(Date, "month")
  ) %>%
  group_by(Date) %>%
  summarise(Units = sum(Units.x),
            best_baseline = sum(best_baseline)
  )

# Ensure Date column is recognized as Date type

# Visualization using ggplot2
ggplot(merged_data_for_viz, aes(x = Date)) +
  geom_line(aes(y = Units, color = "Actual Sales")) +
  geom_line(aes(y = best_baseline, color = "BEST_BL")) +
  labs(title = "Actual Sales vs Baselines", x = "Date", y = "Units") +
...
dbExecute(con, "DELETE FROM ceran.baseline_model_results WHERE \"Client\" = 'Inkafarma'")
